...
abstract class RestController extends CI_Controller
{
    ...
    */
    ...
    public function _remap($object_called, $arguments)
    {
        ...
        if (extension_loaded('newrelic')) { // For New Relic And Ensure PHP agent is available
            newrelic_name_transaction($controller_method);
        }
        // And...... GO!
     ...
     */
    public function response($data = null, $http_code = null, $cacheControl = 'no-cache, no-store, must-revalidate')
    {
        ...
        $http_code_msg = '';
        switch($http_code) {
            case 200: $http_code_msg .= 'OK';
                break;
            case 301: $http_code_msg .= 'Moved Permanently';
                break;
            case 400: $http_code_msg .= 'Bad Request';
                break;
            case 401: $http_code_msg .= 'Unauthorized';
                break;
            case 402: $http_code_msg .= 'Payment Required';
                break;
            case 403: $http_code_msg .= 'Forbidden';
                break;
            case 404: $http_code_msg .= 'Not Found';
                break;
            case 405: $http_code_msg .= 'Method Not Allowed';
                break;
            case 500: $http_code_msg .= 'Internal Server Error';
                break;
            case 502: $http_code_msg .= 'Bad Gateway';
                break;
            case 503: $http_code_msg .= 'Service Unavailable';
                break;
            case 504: $http_code_msg .= 'Gateway Timeout';
                break;
        }

        //die;
        header('HTTP/1.1: ' . $http_code.' '.$http_code_msg);
        header('Status: ' . $http_code.' '.$http_code_msg);
